...
**Coding Agent에게 진짜 원하는 걸 전하고 — Agent가 실제로 한 일을 이해하세요.**

평소 말대로 원하는 걸 말하면 됩니다. 작업 전에는 결과를 바꿀 만한 모호함만 짚고, 프로젝트에서 직접 확인할 수 있는 건 묻지 않고 확인한 뒤, 진짜 결정이 남을 때만 최대 한 가지만 묻습니다. 작업 후에는 뭐가 바뀌었는지, 진짜로 확인된 건 뭔지, 아직 모르는 건 뭔지, 다음에 뭘 하면 되는지를 비개발자도 바로 행동할 수 있는 말로 보고합니다.

![사용자와 JuTell, Coding Agent가 양방향으로 함께 동작합니다. 작업 전에는 JuTell이 원하는 것을 분명하게 만들고, 작업 후에는 Agent가 한 일을 설명합니다.](docs/assets/readme/user-journey.ko.svg)

**작업 전** — 결과를 바꾸는 결정만 분명히 하고, 기술적인 질문 대신 프로젝트 사실을 직접 확인하며, 요청한 범위를 지킵니다.

**작업 후** — 뭐가 바뀌었는지 설명하고, 확인됨·예상·미확인을 엄격히 나눠서 보여주며, 아직 여러분 확인이 필요한 것을 알려줍니다.

JuTell은 AI 모델도, IDE도, Agent GUI도 아닙니다. Agent를 대신하거나 작업을 중간에서 전달하지도, Agent의 답이 맞다고 보증하지도 않습니다 — 이미 쓰는 Agent 옆에 붙어 **양방향으로 정확하게 이어주는** 역할만 합니다. 사용자가 Agent에게 말하고, Agent가 사용자에게 답하는 그 사이를 돕습니다.
...
`2.0.0`은 이 저장소의 현재 소스 버전입니다 (`jutell@2.0.0`, 릴리스 후보 — 아직 npm에 공개되지 않음).
</details>
...
1. 평소 말대로 요청합니다 — 예: "회원가입 좀 간단하게 해줘."
2. JuTell은 결과를 바꿀 만한 모호함만 짚습니다.
3. Agent는 여러분에게 묻기 전에 프로젝트에서 직접 확인할 수 있는 것(지금 화면, 입력 항목, 코드)부터 확인합니다.
4. 진짜 결정이 남으면 짧게 한 가지만 묻습니다 — 기술 퀴즈는 없습니다.
5. Agent는 요청한 것과, 그에 꼭 필요한 관련 작업만 바꿉니다.
6. 검증은 여러분이 실제로 요청한 결과 기준입니다 — 있다고 해서 관련 없는 전체 테스트를 돌리지 않습니다.
7. 짧은 보고서를 받습니다: 뭐가 바뀌었는지, 확인된 건 뭔지, 아직 모르는 건 뭔지, 여러분이 뭘 하면 되는지.

### Agent가 작업을 끝내면, 진짜 궁금한 것에 답합니다
...
| 여러분의 말 | JuTell + Agent가 하는 일 |
|---|---|
| "회원가입 좀 간단하게 해줘." | 먼저 지금 가입 화면 구조를 확인합니다 — "어떤 프레임워크인가요?" 같은 질문은 없습니다. |
| 위험한 추측: "간단하게"가 뭘까? | 결과를 바꾸는 결정만 묻습니다 (예: 화면만 다듬을지, 입력 항목을 줄일지) — 한 번 묻고 바로 작업합니다. |
| 조용한 위험: 범위 넓어지기. | 허락된 범위만 바꾸고, 요청 기준으로 검증한 뒤, 나머지는 정직하게 보고합니다. |

외울 작업 흐름도는 없습니다 — 위의 하나의 흐름과 같습니다. 중요한 것만 분명히 하고, 요청한 것만 바꾸고, 관련된 것만 검증합니다.
...
JuTell이 약속하지 않는 일: Agent가 여러분 말을 정확히 이해했다고 보증하지 않고, 요청과 관련된 모든 의존성을 찾아냈다고 주장하지 않으며, 코드가 맞는지 독자적으로 인증하지 않고, 승인 여부에 대한 여러분 판단을 대신하지 않습니다. 요청을 더 분명하게 만들고 근거를 확인할 수 있게 할 뿐, 마지막 결정은 여러분 것입니다.
...
**`jutell@2.0.0` — 릴리스 후보 (아직 npm에 공개되지 않음)**

JuTell이 이제 Agent가 끝낸 뒤뿐 아니라, *시작하기 전*에도 돕습니다.

- 프로젝트 사실은 직접 확인하고, 결과를 바꾸는 결정만 여러분에게 묻습니다 — 짧게 최대 한 번.
- 요청한 범위를 지킵니다. 요청한 것과 꼭 필요한 관련 작업만 바꾸고, 관련 없는 "개선"은 손대지 않습니다 (써도 한 번만 언급하고, 몰래 하지 않습니다).
- 완료 검증은 여러분이 실제로 요청한 결과 기준이며, 이미 모은 근거를 재사용합니다 — 있다고 해서 관련 없는 전체 테스트를 돌리지 않습니다.
- 읽기 쉬운 보고서는 그대로입니다. 확인됨·예상·미확인을 나누고, 여러분 확인이 필요한 것을 알려줍니다.
- 설치·지원 Agent·지원 환경은 그대로입니다. `npm install -g jutell` 후 `jutell` 실행, Codex 정식 지원, Claude Code·OpenCode 베타.

**`jutell@1.1.0`** (이전 버전, npm에 공개된 버전)
