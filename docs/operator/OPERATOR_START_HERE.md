...
| Private 메모 | 운영자 전용 비공개 저장소 | 아니요 (비공개) | 실험 백로그, 내부 결정 |

**주의**: 비공개 저장소의 실제 내용을 공개 문서에 옮겨 적지 않습니다. 공개 문서에는 일반 원칙만 남깁니다.
...
| 내부 결정 (왜 이렇게 하는가) | 운영자 전용 비공개 저장소 (일반 원칙만) |
...
문제를 발견했다
  └─ 해당 Page의 `내 피드백`에 기록
      ...
      ├─ 새 기능 아이디어 → 비공개 저장소 기록 (원칙만)
      ├─ 왜 이렇게 하지? 결정 질문 → 비공개 저장소 기록 (원칙만)
...
- [공개 제품과 운영자 비공개 자료 경계 정책](../OPERATOR_PRIVATE_DATA_BOUNDARY.md) — 공개 제품과 운영자 비공개 자료의 경계
