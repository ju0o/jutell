...
| `planner/Phase 8`에 "유료 가격 기능" 언급, `docs/PRODUCT_SCOPE.md`에 "유료 팀 기능" 1건 | 분류 완료 | `planner/Phase 8`은 내부 사업·출시 전략으로 판단해 공개 저장소에서 제거, `PRIVATE_DOCUMENTS_MAP.md`에 운영자 전용 비공개 저장소 이동 대상으로 기록. `PRODUCT_SCOPE.md`의 "유료 팀 기능"은 공개 수준인 "팀 기능"으로 축약 |
...
- `planner/Phase 8` 내용을 운영자가 운영자 전용 비공개 저장소로 이동 (공개 저장소에서는 제거됨)
