...
이 설정은 AI Agent Skill이 보고서를 작성할 때 읽는 문서 기반 설정이다. 로컬 관리자와 Distribution CLI가 이 파일을 안전하게 만들거나 관리할 수 있지만, 중앙 서버와 계정 동기화는 현재 범위가 아니다. Provider 연결은 현재 Codex만 지원한다.
...
현재는 프로젝트 루트의 로컬 설정을 기본으로 지원하고, 로컬 관리자와 Distribution CLI에서 사용자 전역 범위도 준비할 수 있다. 중앙 서버, 계정별 동기화, 다른 Provider의 실제 연결, 원격 분석과 사용량 수집은 현재 범위가 아니다. 화면과 문서는 Provider 확장을 고려하지만 현재 연결 대상은 Codex다.
