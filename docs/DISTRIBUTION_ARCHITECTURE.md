...
- Claude Code MCP 설정: `~/.claude.json`의 local(프로젝트, 기본값)·user(전역) 범위 (베타) — `claude mcp add/remove`로 등록
- 전역 관리자 상태: 사용자 홈의 `.jutell-local/`

전역 설치에서 실제 프로젝트의 보고서 설정은 해당 프로젝트의 설정을 우선합니다. 전역 설치는 Skill과 현재 지원 Provider인 Codex 연결을 사용자 범위에 준비하는 방식입니다. OpenCode는 베타로 프로젝트 `opencode.json`에, Claude Code는 베타로 `~/.claude.json`에 JuTell 관리 블록을 등록합니다. 그 외 다른 Provider 연결은 아직 지원하지 않습니다.
...
서버 실행, Provider 설정 등록, 실제 AI Agent 도구 호출은 서로 다른 상태입니다. CLI는 앞의 두 상태를 확인할 수 있지만, 실제 도구 호출은 새 Agent 세션에서 사용자가 직접 확인해야 합니다. 현재 실제 연결 확인 대상은 Codex(지원)와 OpenCode·Claude Code(베타)입니다.
