...
├─ dist/                 # CLI 실행 코드
├─ assets/
│  ├─ skill/             # Beginner Bridge Skill과 reference
...
- 설정: `<프로젝트>/.beginner-bridge.json`
- Codex MCP 설정: `<프로젝트>/.codex/config.toml`
- Beta Journal과 관리자 상태: `<프로젝트>/.beginner-bridge-local/`

### 사용자 전역

- Skill: 사용자 홈의 `.agents/skills/beginner-bridge/`
- 설정: 사용자 홈의 `.beginner-bridge.json`
- Codex MCP 설정: 사용자 Codex 설정의 `config.toml`
- 전역 관리자 상태: 사용자 홈의 `.beginner-bridge-local/`

전역 설치에서 실제 프로젝트의 보고서 설정은 해당 프로젝트의 설정을 우선합니다. 전역 설치는 Skill과 Codex 연결을 사용자 범위에 준비하는 방식입니다.

## 실행 흐름

1. `setup`이 운영체제·Node·Codex 상태를 확인합니다.
2. 기존 설정을 읽고 미리보기를 보여줍니다.
3. Skill reference를 중복 없이 설치합니다.
4. 기본 설정을 만들거나 기존 설정의 필요한 부분만 보완합니다.
5. Codex 설정을 백업하고 Beginner Bridge MCP 관리 블록만 추가합니다.
...
서버 실행, Codex 설정 등록, 실제 Codex 도구 호출은 서로 다른 상태입니다. CLI는 앞의 두 상태를 확인할 수 있지만, 실제 도구 호출은 새 Codex 세션에서 사용자가 직접 확인해야 합니다.
