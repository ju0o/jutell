...
3. `npm run check:public`을 실행해 탐지 결과(비밀정보·절대 경로·비공개 작업 공간 이름·부모 폴더 접근 포함)가 없는지 확인한다.
...
비공개 전략 문서는 공개 저장소가 아닌 운영자 전용 비공개 작업 공간에 보관한다.
공개 저장소에서 비공개 파일을 참조하지 않는다. 경계와 검사 규칙은 [운영자 전용 비공개 작업 공간 경계](OPERATOR_PRIVATE_WORKSPACE_BOUNDARY.md), 보관 대상 종류는 `PRIVATE_DOCUMENTS_MAP.md`를 참고한다.
