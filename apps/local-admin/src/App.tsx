...
import type { Config, ConfigResponse, Feedback, FeedbackInput, Profile, Readiness } from './types/config';
...
export default function App() {
  ...
  const load = async () => { setBusy(true); try { const [config, readinessData, feedbackData] = await Promise.all([api<ConfigResponse>('/api/config'), api<Readiness>('/api/readiness'), api<{ feedback: Feedback[] }>('/api/feedback')]); setData(config); setReadiness(readinessData); setDraft(cloneConfig(config.config)); setFeedback(feedbackData.feedback); } catch (caught) { setMessage({ text: caught instanceof Error ? caught.message : '로컬 데이터를 읽지 못했습니다.', tone: 'error' }); } finally { setBusy(false); } };
  useEffect(() => { void load(); }, []);
  ...
  const selectProfile = (profile: Profile) => { const preset = PROFILE_CATALOG[profile]; updateDraft({ version: 1, profile, features: { ...preset.features }, limits: { ...preset.limits } }, 'profile'); };
