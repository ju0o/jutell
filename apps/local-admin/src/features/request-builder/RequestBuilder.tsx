...
export function RequestBuilder({ config, onMessage }: { config: Config; onMessage: (text: string, tone: 'success' | 'error') => void }) {
  ...
  return <section>
    ...
    <div className="panel"><h3>V2 예정</h3><p>현재는 템플릿 보기와 복사만 제공합니다. 화면에서 직접 선택·작성하는 기능과 민감정보 경고는 V2에서 추가될 예정입니다. 자동 전송은 V2 설계 대상이 아닙니다.</p><p className="muted">로컬 사용량 실험과 요청 작성 방식 비교는 기본 꺼짐 상태로 준비 중입니다. 자세한 내용은 docs/USAGE_EFFICIENCY_EXPERIMENTS.md를 참조하세요.</p></div>
  </section>;
}
