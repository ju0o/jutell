import type { ConfigResponse, Readiness } from '../../types/config';
...
export function Overview({ data, readiness, showGuide, onDismissGuide, onShowGuide, onCopyPrompt }: { data: ConfigResponse; readiness: Readiness; showGuide: boolean; onDismissGuide: () => void; onShowGuide: () => void; onCopyPrompt: () => void }) {
  ...
  return <section>
    ...
    {showGuide && <div className="first-use-guide panel"><div><p className="eyebrow">처음 사용 안내</p><h3>3단계로 시작하세요</h3></div><ol><li><strong>원하는 Profile 선택</strong><span>보고서의 기본 스타일을 고릅니다.</span></li><li><strong>필요 없는 Feature 끄기</strong><span>끄기 전 무엇이 줄어드는지 확인합니다.</span></li><li><strong>저장 후 시작 프롬프트 전달</strong><span>Codex가 이 설정을 읽도록 안내합니다.</span></li></ol><button className="text-button" onClick={onDismissGuide}>안내 닫기</button></div>}
