...
import { AGENT_PROVIDER_CATALOG, CURRENT_AGENT_PROVIDER } from '../../lib/providers';
...
function Check({ ok, children }: { ok: boolean; children: React.ReactNode }) {
  return <li className={ok ? 'check-ok' : 'check-missing'}><span aria-hidden="true">{ok ? '✓' : '!'}</span>{children}</li>;
}
...
export function Overview({ data, readiness, mcpStatus, showGuide, onDismissGuide, onShowGuide, onCopyPrompt }: { data: ConfigResponse; readiness: Readiness; mcpStatus: McpStatus; showGuide: boolean; onDismissGuide: () => void; onShowGuide: () => void; onCopyPrompt: () => void }) {
  ...
  return <section>
    ...
    <div className="stat-grid">
      <div className="stat-card"><span>현재 Profile</span><strong>{PROFILE_CATALOG[data.config.profile].label}</strong><small>{data.config.profile}</small></div>
    ...
    <div className="panel readiness-panel"><div className="section-heading"><div><p className="eyebrow">AI Agent 적용 상태</p><h3>{ready ? '이 저장소에서는 JuTell 적용 준비가 되어 있습니다.' : '적용에 필요한 파일을 확인하세요.'}</h3></div><span className={`status-pill ${ready ? 'success' : 'warning'}`}>{ready ? '준비됨' : '추가 확인 필요'}</span></div><p className="muted">Provider 설정 등록과 실제 도구 호출은 서로 다른 상태입니다. 실제 호출을 확인하지 않았다고 설치 실패로 판단하지 않습니다.</p><div className="provider-summary"><strong>현재 연결 Provider: {CURRENT_AGENT_PROVIDER.label}</strong><span>{CURRENT_AGENT_PROVIDER.description} 다른 Agent도 같은 구조로 연결할 수 있도록 화면을 분리해 두었습니다.</span><small>확장 준비: {AGENT_PROVIDER_CATALOG.filter((provider) => provider.status === 'planned').map((provider) => provider.label).join(', ')}</small></div><ul className="check-list"><Check ok={readiness.config.exists && readiness.config.valid}>설정 파일: {readiness.config.exists && readiness.config.valid ? '정상' : '확인 필요'} · 활성 Feature {readiness.config.activeFeatures}개</Check><Check ok={readiness.skill.exists}>Skill: {readiness.skill.exists ? '발견' : '확인 필요'}</Check><Check ok={readiness.agents.exists && readiness.agents.jutellBlock}>AGENTS.md JuTell 블록: {readiness.agents.jutellBlock ? '확인됨' : '확인 필요'}</Check><Check ok={readiness.safetyRules.exists}>안전 강제 보고 규칙: {readiness.safetyRules.exists ? '발견' : '확인 필요'}</Check><Check ok={mcpStatus.preparation === 'enabled'}>AI Agent 연결 준비: {preparationLabel}</Check><li className="check-neutral"><span aria-hidden="true">i</span>실제 도구 호출: 확인하지 않음</li></ul><p className="callout">{mcpStatus.preparation === 'enabled' ? '새 AI Agent 세션부터 사용할 수 있습니다. 실제 도구 호출 여부는 해당 Agent에서 확인할 수 있습니다.' : 'MCP를 사용하지 않아도 Skill 방식은 계속 사용할 수 있습니다.'}</p><div className="prompt-box"><div className="section-heading"><strong>복사해서 시작하기</strong><button className="secondary small" onClick={onCopyPrompt}>시작 프롬프트 복사</button></div><pre>{prompt}</pre></div></div>
