...
const server = new McpServer({
  ...
  instructions: 'Beginner Bridge is a local read-only report helper. Read only the project configuration and approved report rules. Never access project code, Git diff, prompts, AI answers, secrets, or external networks. Skill mode remains available if this MCP server is disabled or unavailable.',
});

const noInput = { inputSchema: {} } as const;

server.registerTool('get_bridge_status', { ...noInput, description: 'Return local Beginner Bridge configuration and availability status.' }, async () => {
  const context = await readBridgeContext();
  return { content: [{ type: 'text', text: JSON.stringify(bridgeStatus(context)) }] };
});

server.registerTool('get_active_features', { ...noInput, description: 'Return active Beginner Bridge features and their safe omissions.' }, async () => {
  const context = await readBridgeContext();
