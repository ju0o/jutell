...
export async function getStatus(paths: ScopePaths): Promise<StatusResult> {
  ...
  if (config.config.mcp?.enabled && !registration.registered) warnings.push('MCP가 켜져 있지만 Codex 설정이 등록되지 않았습니다.');
  return {
    ...
    skillInstalled: await exists(path.join(paths.skillRoot, 'SKILL.md')),
...
export async function getDoctorResults(paths: ScopePaths): Promise<CheckResult[]> {
  ...
  checks.push({ name: '설정 백업 상태', status: !await exists(paths.codexConfigFile) || backupExists ? '정상' : '주의', detail: !await exists(paths.codexConfigFile) ? '아직 Codex 설정을 변경하지 않았습니다.' : backupExists ? '이전 설정 백업을 확인했습니다.' : '기존 설정 백업이 없습니다.' });
