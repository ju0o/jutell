...
이번 요청이 실제로 원하는 결과를 만들어냈는지, 사용자가 말한 건드리지 말 것·유지할 것·완료 조건이 실제로 유지됐는지 확인하지 못했다면 `확인 완료`를 쓰지 않는다. `보고서 상태`를 `확인 완료`로 쓰는 모든 보고서는 그 바로 앞에 `완료 판정`을 반드시 적는다 — 완료에 필수적으로 확인되지 않은 것이 정말 없으면 `완료 판정: 없음`, 있으면 `완료 판정: <무엇이 미확인인지>`라고 적고 `보고서 상태`는 `확인 완료`가 아닌 실제로 맞는 상태로 쓴다. 관련 검증이 통과했다는 사실만으로 `완료 판정: 없음`이라고 쓰지 않는다.
...
- 근거: <짧은 이유>
- <필요한 사용자 행동>
- 완료 판정: <없음 또는 무엇이 미확인인지>
- 보고서 상태: <상태>
```

단순 작업은 기본 항목마다 1~2문장, 주요 파일 최대 3개, 전체 기본 12문장 또는 약 25줄을 우선한다. 안전 문제나 중요한 실패가 있으면 정확한 경고를 우선한다.

`완료 판정`은 `보고서 상태`를 정하기 직전에 실제로 판단한 결과를 적는 항목이며, 생략할 수 없다. 이번 요청 결과가 실제로 성공하거나 사용자가 말한 보존·완료 조건이 실제로 성립하는 데 필수적인 항목을 이미 알고 있는데 그것이 확인되지 않았거나 실패했으면 `완료 판정: <무엇이 미확인·실패 상태인지 한 문장>`이라고 적는다(예: "외부 신원 인증 서비스 실제 연결", "실제 결제 승인 성공 여부"). 그런 항목이 정말 없을 때만 정확히 `완료 판정: 없음`이라고 쓴다. `완료 판정`이 `없음`이 아니면 `보고서 상태`에 `확인 완료`를 쓸 수 없다 — 관련 테스트나 검증이 통과했다는 사실만으로 `없음`이라고 쓰지 않는다.

(이전 버전은 완료에 필수적인 미확인이 있을 때만 별도 `완료에 필수적인 미확인` 항목을 만들었다. 그 항목은 생략할 수 있었고, 실제로 자주 생략됐다. `완료 판정`이 그 역할을 대신하며 모든 `확인 완료` 보고서에 예외 없이 필요하다.)
