...
이번 요청이 실제로 원하는 결과를 만들어냈는지, 사용자가 말한 건드리지 말 것·유지할 것·완료 조건이 실제로 유지됐는지 확인하지 못했다면 `확인 완료`를 쓰지 않는다. 완료에 필수적인 미확인·실패를 이미 알고 있다면 아래 보고서 형식의 `완료에 필수적인 미확인` 항목에 적는다 — 그 항목이 있으면 관련 검증이 통과했다는 사실만으로 `확인 완료`를 쓰지 않는다.
...
`완료에 필수적인 미확인`은 이번 요청 결과가 실제로 성공하거나 사용자가 말한 보존·완료 조건이 실제로 성립하는 데 필수적인 항목을 이미 알고 있는데 그것이 확인되지 않았거나 실패했을 때만 쓴다. 값은 "있음"처럼 비워두지 않고 무엇이 미확인·실패 상태인지 한 문장으로 짧게 적는다(예: "외부 신원 인증 서비스 실제 연결", "실제 결제 승인 성공 여부"). 완료에 필수적인 미확인이 없으면 이 항목 자체를 만들지 않는다 — 값이 있을 때만 보여주는 다른 필드와 같다. 이 항목이 있으면 바로 다음 줄의 `보고서 상태`에 `확인 완료`를 쓸 수 없다.
