...
## 5. 중요한 코드 (Readable Code)

중요한 코드 블록이 이미 작업 과정에서 확인되었고, 사용자 이해에 실제 도움이 될 때만 1~2개까지 보여준다. 한 보고에서 절대 상한은 12개이지만, 기본은 1~2개다.

형식 예시:

- `[중요한 코드]` 뒤에 짧은 코드 블록 1개
- `[쉽게 보면]`에 비개발자용 한두 문장
- `[영향]`에 사용자에게 달라지는 점 한두 문장

예시 코드: `if (!query.trim()) return;`
쉽게 보면: 빈칸이나 공백만 입력한 상태에서는 검색이 실행되지 않도록 막는 부분입니다.
영향: 사용자가 공백만 입력했을 때 불필요한 검색 요청이 발생하지 않습니다.

규칙:

* 이미 읽거나 비교한 코드만 사용한다.
* 이 섹션만을 위해 파일을 다시 읽지 않는다.
* 이 섹션만을 위해 git diff를 다시 실행하지 않는다.
* 코드 근거가 없으면 이 섹션을 만들지 않는다.
* 긴 raw diff를 반복하지 않는다.
* 줄별 프로그래밍 문법 강의를 하지 않는다.
* 인증·권한·결제·데이터베이스처럼 민감한 영역은 보여주더라도 주의가 필요하다고 표시한다.
* `explainedDiff`가 OFF이면 Readable Code도 생략한다. 사용자가 코드 설명을 명시적으로 요청한 경우만 예외다.
...
[쉽게 보면]
빈칸이나 공백만 입력한 상태에서는 검색이 실행되지 않도록 막는 부분입니다.

[영향]
사용자가 공백만 입력했을 때 불필요한 검색 요청이 발생하지 않습니다.
```
