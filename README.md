...
주요 명령은 `jutell setup`, `jutell status`, `jutell doctor`, `jutell enable`, `jutell disable`, `jutell uninstall`입니다. 기존 사용자를 위해 `beginner-bridge`도 같은 기능의 호환 별칭으로 유지합니다.

## Skill, MCP, 관리자

Skill은 기본 보고 경로입니다. `.agents/skills/beginner-bridge/SKILL.md`는 호환성을 위해 ID와 경로를 유지하고 표시 이름은 JuTell로 바꿉니다. 프로젝트 설정은 `.jutell.json`을 우선 읽고, 없으면 기존 `.beginner-bridge.json`을 읽습니다.

MCP는 선택형 로컬 읽기 연결입니다. MCP가 꺼지거나 실패해도 Skill 방식은 계속 사용할 수 있습니다. MCP 서버 실행, Codex 설정 등록, 실제 도구 호출은 서로 다른 상태입니다.

로컬 관리자는 Profile, Feature, limits, Beta Journal, MCP 연결 준비 상태를 현재 컴퓨터에서만 관리합니다.
